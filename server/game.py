...
class Game:
    def __init__(self, TCPAddr, webAddr):
        ...
        self.restorePlayers()
        self.TCPserver = Server(TCPAddr, self.processClientData)
        self.webServer = ServerHTML(host=webAddr[0], port=webAddr[1], game=self)
    ...
    def processClientData(self, playerMessage, adress):
        if playerMessage.startswith('login:'):
            ...
            if playerMessage in ['interact:show', 'getMap', 'getPos', 'resetActions', 'getInv']:
